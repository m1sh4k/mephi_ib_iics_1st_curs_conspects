...
3. $\emptyset$ —  нет элементов $\forall A:\ \emptyset \subset A$ 
   - Док-во (от противного): пусть $\exists A$ такое, что $\emptyset \not\in A$, т.е. $\exists a\in \emptyset: \ a \not\in A$ (противоречие)

## Действия над множествами

1. $A + B$  (или $A  B$) все элементы, которые прин. A и B
   $A \bigsqcup B$ — дизъюнктивное объединение 
...
4. $A \bigtriangleup  B=(A\lor B) \setminus (A\cap B)$  — симметрическая разность
## Законы Моргана
   ...
   - Док-во:
     - a) $A+BC\subset(A+B)(A+C)$
		  - $\forall x \in A +BC$
			   ...
			   2) $x\in BC\implies x\in B, \ x\in C \implies \forall A \ \ x\in(A+B), \ x\in(a+C)\implies x \in (A+B)(A+C)$
